...
<body class="bg-black text-white container mx-auto grid place-items-center">
    ...
    <br>
    <br>
    <div class="border border-blue-500 border-solid w-[650px] py-4 px-3 rounded">
        <h2 class="text-blue-400 text-2xl">All Posts</h2>
        @foreach ($posts as $post)
            <div class=" bg-slate-500 my-4 p-2 rounded hover:scale-[1.01] transition flex justify-between items-center">
                <div>
                    <h3>{{$post['title']}}
                    </h3>
                    <p>{{$post['body']}}</p>
                    <small class="text-slate-800">{{$post['created_at']}}</small> <span class="text-slate-800 text-sm">
                         by {{$post->user->name}}
                        </span>
                </div>
            <div class="flex gap-2">
                <a class="py-2 px-4 bg-blue-500 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75" href="/edit-post/{{$post->id}}"><button>Edit</button></a>
                <form action="/delete-post/{{$post->id}}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button class="py-2 px-4 bg-red-500 text-white font-semibold rounded-lg shadow-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-opacity-75">Delete</button>
                </form>
                </div>
            </div>
        @endforeach
    ...
    <div class="border-3 border-solid border-white border p-4 py-10 w-[500px] mx-auto rounded-s">
        ...
        <br>
        <p>have an account? log in <a href="{{url('/login')}}" class="underline text-blue-500">here</a></p>
    </div>
